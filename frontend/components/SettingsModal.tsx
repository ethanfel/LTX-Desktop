              ...
              <div className="space-y-3 pt-4 border-t border-zinc-800">
                <div className="flex items-start justify-between gap-4">
                  <div className="flex-1">
                    ...
                    <p className="text-xs text-zinc-500 leading-relaxed">
                      Save lossless PNG frames alongside each generated video. Used as higher-quality source for retake and blend operations. Uses significant disk space (~1-2 GB per video at 1080p).
                    </p>
                  </div>

                  <button
                    onClick={() => onSettingsChange({ ...settings, savePngFrames: !settings.savePngFrames })}
                    className={`relative inline-flex h-6 w-11 flex-shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none ${
                      settings.savePngFrames ? 'bg-cyan-500' : 'bg-zinc-700'
                    }`}
                  >
                    <span
                      className={`pointer-events-none inline-block h-5 w-5 transform rounded-full bg-white shadow ring-0 transition duration-200 ease-in-out ${
                        settings.savePngFrames ? 'translate-x-5' : 'translate-x-0'
                      }`}
                    />
                  </button>
                </div>
