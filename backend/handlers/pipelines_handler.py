...
class PipelinesHandler(StateHandlerBase):
    ...
    def _create_video_pipeline(
        ...
        checkpoint_path = str(resolve_model_path(self.models_dir, self.config.model_download_specs, "full_checkpoint" if model_type in ("pro", "hq") else "checkpoint"))
    ...
    def load_gpu_pipeline(
        ...
        if state is None:
            ...
            state = self._create_video_pipeline(model_type, lora_path=lora_path, lora_strength=lora_strength)
            with self._lock:
